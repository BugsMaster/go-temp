...
func GETQuoteRealTime(c *gin.Context) {
	...
	for {
		...
		nowTime := <-neTicker.C
		currenTimeUnix := time.Now().Unix()
		var actress model.ETFRealTimeData
		//dataUrl:="https://quotedata.cnfin.com/quote/v1/real?en_prod_code=159755.SZ&fields=hq_type_code,trade_status,prod_name,prod_code,data_timestamp,last_px,px_change,preclose_px,open_px,high_px,low_px,amplitude,up_px,down_px&localDate=1664172428066"
		dataUrl := "https://quotedata.cnfin.com/quote/v1/real?en_prod_code=510310.SS&fields=hq_type_code,trade_status,prod_name,prod_code,data_timestamp,last_px,px_change,preclose_px,open_px,high_px,low_px,amplitude,up_px,down_px&localDate=1664172428066" + strconv.FormatInt(currenTimeUnix, 10)
		res, _ := http.Get(dataUrl)
		defer res.Body.Close()
		body, _ := ioutil.ReadAll(res.Body)
		err := json.Unmarshal(body, &actress)
